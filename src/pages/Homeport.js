...
function Homeport() {
  ...
  // useRef for scrolling to skills section
  const skillsSectionRef = useRef(null);

  // Memoize skills list to avoid unnecessary recalculations
  const memoizedSkills = useMemo(() => skills, [skills]);

  // Memoize handler to avoid re-creating on each render
  const handleSectedSkill = useCallback((skill) => {
    ...
    <>
      <section className="home-container">
        <div className="home-content">
          <h2>{data.name}</h2>
          <p>{data.bio}</p>
          <button onClick={scrollToSkills}>See My Skills</button>
        </div>
        <div className="home-img">
          <div>
            <div className="tech-icon">
      ...
      <section className="skills-container" ref={skillsSectionRef}>
        <h5>Technical Proficiency</h5>
        <div className="skills-content">
          <div className="skills">
            {memoizedSkills.map((skill) => (
              <Skillscard
                ...
                title={skill.title}
                isActive={selectedSKill.title === skill.title}
                onClick={() => handleSectedSkill(skill)}
              />
            ))}
          </div>
          <div className="skills-info">
